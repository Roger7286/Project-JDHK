"""
fetch_transinbill.py
====================
职责：Token 管理 + 调用金蝶分步调入单查询接口（/v2/im/im_transinbill/getList）
      返回接口原始数据，不做任何字段转换，不涉及数据库。

对外暴露：
    get_valid_token()      → str         获取有效 access_token
    query_transinbill()    → dict        单页查询，返回接口原始 JSON
...
_API_PATH           = API_PATHS["transinbill"]
...
def query_transinbill(
    ...
    org_number:         list = None,   # Array<String>：调入组织编码列表
    pageNo:             int  = 1,
    pageSize:           int  = None,
) -> dict:
    """
    调用金蝶分步调入单查询接口（单页），返回接口原始 JSON dict。
    接口路径: /v2/im/im_transinbill/getList
...
if __name__ == "__main__":
    ...
    _parser = _argparse.ArgumentParser(
        description="fetch_transinbill.py — 独立测试，只查询不落库",
    ...
    print("=" * 60)
    print("【fetch_transinbill.py 独立测试 — 只查询不落库】")
    print("=" * 60)
