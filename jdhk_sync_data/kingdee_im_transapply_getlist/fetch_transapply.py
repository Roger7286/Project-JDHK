...
fetch_transapply.py
===================
职责：Token 管理 + 调用金蝶调拨申请单查询接口（/v2/im/im_transapply/getList）
      返回接口原始数据，不做任何字段转换，不涉及数据库。

对外暴露：
    get_valid_token()      → str         获取有效 access_token
    query_transapply()     → dict        单页查询，返回接口原始 JSON
...
_API_PATH           = API_PATHS["transapply"]
...
def query_transapply(
    ...
    org_number:         list = None,   # Array<String>：申请组织编码列表
    pageNo:             int  = 1,
    pageSize:           int  = None,
) -> dict:
    """
    调用金蝶调拨申请单查询接口（单页），返回接口原始 JSON dict。
    接口路径: /v2/im/im_transapply/getList

    参数说明：
        start_biztime    业务起始日期（"2024-06-01"，必填之一）
        end_biztime      业务截止日期
        start_auditdate  审核起始时间（"2024-06-01 00:00:00"，可选）
        end_auditdate    审核截止时间
        billno           单据编号列表（可选，如 ["DBSQ-001","DBSQ-002"]）
        billstatus       单据状态列表（可选，如 ["A","B","C"]）
        org_number       申请组织编码列表（可选）
        pageNo           页码，从 1 开始
        pageSize         每页条数
    """
    if pageSize is None:
...
# =========================================================
# 独立测试（直接运行此文件时）
# 支持命令行参数；未传则用动态默认值（今天-30天 ~ 今天）
#
# 用法示例：
#   python fetch_transapply.py
#   python fetch_transapply.py --billno "DBSQ-260405-000001"
#   python fetch_transapply.py --start_biztime "2025-01-01" --end_biztime "2025-03-31"
#   python fetch_transapply.py --billstatus C --pagesize 5
# =========================================================
...
if __name__ == "__main__":
    ...
    _parser = _argparse.ArgumentParser(
        description="fetch_transapply.py — 独立测试，只查询不落库",
    ...
    print("=" * 60)
    print("【fetch_transapply.py 独立测试 — 只查询不落库】")
    print("=" * 60)
