...
def parse_args():
    """解析命令行参数，所有参数均为可选，不传时使用 SYNC_PARAMS 默认值。"""
...
def main():
    ...
    # 所有参数：命令行传了就用命令行，否则用 SYNC_PARAMS 默认
    ...
    org              = args.org              if args.org              is not None \
                       else SYNC_PARAMS.get("org")
    org_number       = args.org_number       if args.org_number       is not None \
                       else SYNC_PARAMS.get("org_number")

    # 打印本次同步参数
    print(f"\n同步参数：")
    print(f"  modifytime_start : {modifytime_start or '（未设置，全量）'}")
    print(f"  modifytime_end   : {modifytime_end   or '（未设置，全量）'}")
    print(f"  org              : {org or '（未设置）'}")
    print(f"  org_number       : {org_number or '（未设置）'}")
